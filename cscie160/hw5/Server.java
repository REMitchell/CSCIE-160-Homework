...
/*The server process will start up, create an
 *  ATMFactoryImpl instance, and then register
 *   it with the RMI naming service or registry 
 *   using java.rmi.Naming. The client will then be 
 *   able to lookup the ATM factory and connect to it. 
 *   Then the client will use the getATM() method of 
 *   the factory to get a remote reference to an ATM 
 *   instance.
 */
/*
 * Create a new Server class. The Server is used to 
 * startup the server process. Since you've created a 
 * convenient factory object, the only work the Server
 *  has to do is create an instance of the factory, export 
 *  it and register it with the RMI registry.
 */
